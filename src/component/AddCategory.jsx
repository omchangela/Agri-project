...
const AddCategory = () => {
  ...
  const fetchCategories = async (token) => {
    try {
      const response = await axios.get('https://ottb.leadgenadvertisements.com/api/category/v1/category', {
        headers: {
  ...
  return (
    <div className="flex items-center justify-center flex-col min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-xs mb-8">
        <h2 className="text-2xl font-bold text-center mb-14">Add Category</h2>
        ...
        <div {...getRootProps()} className="border-2 border-dashed border-gray-300 p-4 text-center cursor-pointer rounded">
          ...
          <div className="mb-4">
            <label className="block text-gray-700">Confirm Category:</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
            ...
            {categories.map((category, index) => (
              <tr key={category.id} className="text-center">
                ...
                <td className="p-2 border-b">
                  <img
                    src={category.imageUrl} // Assuming 'imageUrl' is the field for the image path
                    alt={category.name}
                    className="w-12 h-12 object-cover mx-auto rounded"
                  />
                </td>
                <td className="p-2 border-b">{category.status ? 'Active' : 'Inactive'}</td>
              </tr>
            ))}
          </tbody>
        </table>
